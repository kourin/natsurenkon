...
rem 一般的には、AACの場合、320kbps以上に高くしても違いが聴き分けられないと言われています
rem 耳のすごく良い人が、すごく良い視聴環境で聴けば、もしかしたら違いがわかるのかもしれません
...
rem neroかqt（iTunesまたはQuickTimeがインストールされてる必要があります）かを選択
...
rem 自動エコ回避
rem あと少し音声ビットレートを下げれば、エコノミー回避出来そうな場合に、自動でエコノミー回避するかどうか
rem (YouTube用プリセット(y)では自動で無効化。また、ビットレートの質問でファイルサイズを指定した場合も無効)
rem デフォルトはy (有効)。無効にしたい場合は、n
set AUTOECO=y
